...
void parse(FILE *file) {
    ...
    int rom_address = 0;        // ROM address counter
    char inst_type;             // keep inst_type, as requested
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        // ---------------------------
        if (is_Atype(line)) {
            inst_type = 'A';    // kept, but no longer used
            // printf("%c %s\n", inst_type, line);   // commented out

            rom_address++;      // A-instruction consumes ROM address
            continue;
        }

        if (is_label(line)) {
            inst_type = 'L';    // kept, but no longer used
            // printf("%c %s\n", inst_type, line);   // commented out

            char label[MAX_LABEL_LENGTH];
            extract_label(line, label);

            // Store label -> address in symbol table
        ...
        if (is_Ctype(line)) {
            inst_type = 'C';    // kept, but no longer used
            // printf("%c %s\n", inst_type, line);   // commented out

            rom_address++;      // C-instruction consumes ROM address
            continue;
        }
    }
}
